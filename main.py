...
import Ip_Get_class as ip_class
...
class Producer(threading.Thread):
    #初始化
    def __init__(self,url,page_all,work_queue,ip_list):
        super().__init__()
...
class house_info(threading.Thread):
    #初始化参数
    def __init__(self,work_queue,ip_list):
        super().__init__()
        self.work_queue = work_queue
        self.ip_list = ip_list
        self.headers = {'User-Agent':'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36'}
...
def main(ip_list):
    ...
    thread_first = Producer(url_index,100,work_queue,ip_list)
    thread_first.daemon = True  # 当主线程退出时子线程也退出
    thread_first.start()

    thread_second = house_info(work_queue,ip_list)
...
if __name__ == '__main__':
    ip_get = ip_class.Ip_getting(Web_Info)
    main(ip_get.output_ip())
